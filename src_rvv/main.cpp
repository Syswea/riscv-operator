...
void flash_atten(HBM &hbm) {
    ...
    for (size_t i = 0; i < N; i += br) {
        ...
        for (size_t j = 0; j < N; j += bc) {
            ...
            // printf("Output block sram.S first:\n");
            // Memprint(sram.S, br * bc);
            update_sml(sram.S, sram.m_old, sram.m_new, sram.l);
            // printf("Output vector sram.l:\n");
            // Memprint(sram.l, br);
            // printf("Output block sram.S second:\n");
            // Memprint(sram.S, br * bc);
            compute_pv(sram.O, sram.S, sram.V, sram.m_old, sram.m_new);
            // printf("Output block sram.O first:\n");
            // Memprint(sram.O, br * d);
        }
        scale(sram.O, sram.l);
        // printf("Output vector sram.l:\n");
        // Memprint(sram.l, br);
        // printf("Output block sram.O second:\n");
        // Memprint(sram.O, br * d);
...
int main() {
        ...
        printf("RVV Intrinsic version: %d\n", __riscv_v_intrinsic);
    #else
        printf("RVV Intrinsics not defined.\n");
    #endif
